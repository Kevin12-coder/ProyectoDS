...
export class UsersService {
  ...
  async canDo(user: UserEntity, permission_id: number): Promise<boolean> {
    if (user.permissions.find((PermissionEntity) => PermissionEntity.id === permission_id)) {
      return true;
    } else if (
      user.roles.find((RoleEntity) => RoleEntity.permissions.find((PermissionEntity) => PermissionEntity.id === permission_id))
    ) {
      return true;
    } else {
      return false;
